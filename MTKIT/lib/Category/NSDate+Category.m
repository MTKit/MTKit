...
// 会话页面时间显示处理
- (NSString *)conversationFormatTime
{
    ...
    {//间隔一周内
        ...
        NSString *timeStr = [inputFormatter stringFromDate:self];
...
- (NSString *)chatFormatTime{
    ...
    else if ([self isInSevenDays])
    {
        ...
        return [NSString stringWithFormat:@"%@ %@",weekStr,[dateFormatter stringFromDate:self]];
    }
    else
    {
        //以前
...
- (NSString *)timeIntervalSince1970MillisecondString
{
    return [NSString stringWithFormat:@"%.0f",[self timeIntervalSince1970]*1000];
}

- (NSString *)mt_stringValueType:(NSString *)type
{
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
