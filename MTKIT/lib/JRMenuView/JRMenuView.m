...
- (void)show
{
    if (!self.hasShow) {
        ...
        self.hasShow = YES;
        self.frame = CGRectMake(targetView.frame.origin.x-JRMenuTW, targetView.frame.origin.y, 0, JRMenuHeight);
        [UIView animateWithDuration:.1 animations:^{
            [self setFrame:CGRectMake(targetView.frame.origin.x - jrMenuWidth-JRMenuTW, targetView.frame.origin.y, jrMenuWidth, JRMenuHeight)];
        }];
    }else{
        [self dismiss];
    }
...
- (void)dismiss
{
    if (self.hasShow) {
        self.hasShow = NO;
        [UIView animateWithDuration:.1 animations:^{
            [self setFrame:CGRectMake(targetView.frame.origin.x-JRMenuTW, targetView.frame.origin.y, 0, JRMenuHeight)];
        }];
    }
