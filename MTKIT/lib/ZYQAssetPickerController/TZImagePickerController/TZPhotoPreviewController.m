...
- (void)configCustomNaviBar {
    ...
    _selectLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 23, 23)];
    [self.view addSubview:_selectLabel];
    _selectLabel.backgroundColor = [UIColor colorWithRed:0.12 green:0.73 blue:0.13 alpha:1.00];
    _selectLabel.font = [UIFont systemFontOfSize:14];
    _selectLabel.textColor = [UIColor whiteColor];
    _selectLabel.layer.cornerRadius = 23/2;
    _selectLabel.layer.masksToBounds = true;
    _selectLabel.textAlignment = NSTextAlignmentCenter;
    _selectLabel.hidden = true;
    
    _selectLabel.center = _selectButton.center;
    
    [_naviBar addSubview:_selectButton];
    [_naviBar addSubview:_selectLabel];
...
- (void)refreshNaviBarAndBottomBarState {
    ...
    _selectLabel.hidden = !model.isSelected;
    if (model.isSelected) {
        [_tzImagePickerVc.selectedModels enumerateObjectsUsingBlock:^(TZAssetModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (model == obj) {
                _selectLabel.text = [NSString stringWithFormat:@"%zd",idx+1];
                *stop = true;
            }
        }];
    }
