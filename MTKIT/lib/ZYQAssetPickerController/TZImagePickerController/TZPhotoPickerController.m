...
- (void)setSetBottomBarHidden:(BOOL)setBottomBarHidden
{
    _setBottomBarHidden = setBottomBarHidden;
    self.bottomToolBar.hidden = setBottomBarHidden;
}
...
- (void)viewDidLayoutSubviews
{
    ...
    if (self.setBottomBarHidden) {
        _collectionView.frame = self.view.bounds;
    }else{
        _collectionView.height = self.view.height - self.bottomToolBar.height;
    }
}
...
- (void)configBottomToolBar {
    ...
    bottomToolBar.hidden = self.setBottomBarHidden;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.selectPhotoButton.imageView.hidden = model.isSelected;
    cell.selectedLabel.hidden = !model.isSelected;
    
    
    if (model.isSelected) {
        [tzImagePickerVc.selectedModels enumerateObjectsUsingBlock:^(TZAssetModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (obj == model) {
                cell.selectedLabel.text = [NSString stringWithFormat:@"%zd",idx+1];
                *stop = true;
            }
        }];
    }
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        if (isSelected) {
            ...
            if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                ...
                [[Toast shareToast] makeText:@"已达到最大选择数量" aDuration:3];
                return ;
            }
        }
        __weak typeof(tzImagePickerVc)weakTzImagePickerVc = tzImagePickerVc;
        __weak typeof(collectionView)weakCollectionView = collectionView;

        [weakCell.selectPhotoButton setImage:[UIImage imageNamed:@""] forState:UIControlStateSelected];
        
        NSMutableArray <NSIndexPath *> *mArray = [NSMutableArray array];
        for (TZAssetCell *visiableCell in weakCollectionView.visibleCells) {
            if (visiableCell.selectedLabel.hidden == false) {
                NSIndexPath *idp = [weakCollectionView indexPathForCell:visiableCell];
                if (idp) {
                    [mArray addObject:idp];
                }
            }
        }
        
        if (isSelected == false) {
            weakCell.selectImageView.hidden = true;
            weakCell.selectedLabel.text = [NSString stringWithFormat:@"%zd",weakTzImagePickerVc.selectedModels.count];
            weakCell.selectedLabel.hidden = false;
            [UIView showOscillatoryAnimationWithLayer:weakCell.selectedLabel.layer type:TZOscillatoryAnimationToSmaller];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                weakCell.selectImageView.hidden = false;
            });
        }else{
            weakCell.selectPhotoButton.imageView.hidden = false;
            weakCell.selectedLabel.hidden = true;
            [UIView showOscillatoryAnimationWithLayer:weakLayer type:TZOscillatoryAnimationToSmaller];
        }
        
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            if (mArray.count) {
                [weakCollectionView reloadItemsAtIndexPaths:mArray];
            }
        });
    };
    return cell;
