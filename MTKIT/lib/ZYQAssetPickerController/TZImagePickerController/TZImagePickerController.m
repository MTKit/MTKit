...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount delegate:(id<TZImagePickerControllerDelegate>)delegate type:(TZImagePickerType)type {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:4 delegate:delegate pushPhotoPickerVc:YES type:type];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate type:(TZImagePickerType)type {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:columnNumber delegate:delegate pushPhotoPickerVc:YES type:type];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc type:(TZImagePickerType)type {
    _type = type;
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos index:(NSInteger)index type:(TZImagePickerType)type{
    _type = type;
...
- (instancetype)initCropTypeWithAsset:(id)asset photo:(UIImage *)photo completion:(void (^)(UIImage *cropImage,id asset))completion type:(TZImagePickerType)type {
    _type = type;
    TZPhotoPreviewController *previewVc = [[TZPhotoPreviewController alloc] init];
    self = [super initWithRootViewController:previewVc];
    if (self) {
...
- (void)configDefaultSetting {
    ...
    self.allowTakePicture = false; // 在内部显示拍照按钮
    ...
    self.needCircleCrop = false;
    self.circleCropRadius = 100;
    
    self.sortAscendingByModificationDate = true;
    
    self.navigationBar.barTintColor = [UIColor colorWithRed:0.145 green:0.537 blue:0.871 alpha:1.000];
...
- (void)configDefaultBtnTitle {
    ...
    if (self.type == TZImagePickerTypeCircle) {
        self.doneBtnTitleStr = [NSBundle tz_localizedStringForKey:@"完成"];
    }
...
- (void)viewWillAppear:(BOOL)animated {
    [super viewWillAppear:animated];
    TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.navigationController;
    if (imagePickerVc.remeberSelectedModes == false) {
