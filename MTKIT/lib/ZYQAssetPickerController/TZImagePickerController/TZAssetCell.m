...
- (void)layoutSubviews
{
    ...
    self.selectedLabel.frame = CGRectMake(self.contentView.frame.size.width-2-self.selectedLabel.frame.size.width, 2, self.selectedLabel.frame.size.width, self.selectedLabel.frame.size.height);
}
...
- (UIImageView *)videoImgView {
    if (_videoImgView == nil) {
        ...
        videoImgView.frame = CGRectMake(8, 3, 17, 11);
...
- (UILabel *)selectedLabel
{
    if (_selectedLabel == nil) {
        _selectedLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 23, 23)];
        [self.contentView addSubview:_selectedLabel];
        _selectedLabel.backgroundColor = [UIColor colorWithRed:0.12 green:0.73 blue:0.13 alpha:1.00];
        _selectedLabel.font = [UIFont systemFontOfSize:14];
        _selectedLabel.textColor = [UIColor whiteColor];
        _selectedLabel.layer.cornerRadius = 23/2;
        _selectedLabel.layer.masksToBounds = true;
