...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (_delegate!=nil&&[_delegate respondsToSelector:@selector(shouldTap:)]) {
        if (![_delegate shouldTap:self]&&!_selected) {
            return;
        }
    }
...
-(BOOL)shouldTap:(ZYQTapAssetView *)view{
    if (_delegate!=nil&&[_delegate respondsToSelector:@selector(shouldSelectAsset:selected:)]) {
        return [_delegate shouldSelectAsset:_asset selected:view.selected];
    }
    return YES;
}
...
-(BOOL)shouldSelectAsset:(ALAsset *)asset selected:(BOOL)selected{
    if (_delegate!=nil&&[_delegate respondsToSelector:@selector(shouldSelectAsset:selected:)]) {
        return [_delegate shouldSelectAsset:asset selected:selected];
    }
    return YES;
}
...
- (void)setupButtons
{
    ...
    [btn setTitleColor:HEXCOLOR(0xb2b2b2) forState:UIControlStateNormal];
    [btn setBackgroundColor:[UIColor clearColor]];
    btn.titleLabel.font = [UIFont systemFontOfSize:17];
    [btn sizeToFit];
    [btn addTarget:self action:@selector(finishPickingAssets:) forControlEvents:UIControlEventTouchUpInside];
...
- (BOOL)shouldSelectAsset:(ALAsset *)asset selected:(BOOL)selected
{
    ...
    BOOL ret = (selectable && _indexPathsForSelectedItems.count < vc.maximumNumberOfSelection);
    
    if (selected == false) {
        if (_indexPathsForSelectedItems.count == vc.maximumNumberOfSelection) {
             [[Toast shareToast] makeText:@"已达到最大选择数量" aDuration:1];
            if ([vc.delegate respondsToSelector:@selector(assetPickerControllerDidSelectedMaxNumPic)]) {
                [vc.delegate assetPickerControllerDidSelectedMaxNumPic];
            }
        }
    }
